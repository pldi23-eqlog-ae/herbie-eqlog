function fmax(x, y) { if (x != x) { return y; } else if (y != y) { return x; } else { return Math.max(x, y); }}
function fmin(x, y) { if (x != x) { return y; } else if (y != y) { return x; } else { return Math.min(x, y); }}
function pow(x, y) { if (x == 1.0 && isNaN(y)) { return 1.0; } else { return Math.pow(x, y); }}
function fdim(x , y) { if (x != x || y != y) { return NaN; } else if (x > y) { return x - y; } else { return 0; }}

function start(x) {
	return 1.0 - (((1.0 / (1.0 + (0.3275911 * Math.abs(x)))) * (0.254829592 + ((1.0 / (1.0 + (0.3275911 * Math.abs(x)))) * (-0.284496736 + ((1.0 / (1.0 + (0.3275911 * Math.abs(x)))) * (1.421413741 + ((1.0 / (1.0 + (0.3275911 * Math.abs(x)))) * (-1.453152027 + ((1.0 / (1.0 + (0.3275911 * Math.abs(x)))) * 1.061405429))))))))) * Math.exp(-(Math.abs(x) * Math.abs(x))));
}
function end(x) {
	return 1.0 + (Math.exp((x * -x)) * ((1.0 / (1.0 + (0.3275911 * Math.abs(x)))) * (-0.254829592 + ((1.0 / (1.0 + (0.3275911 * Math.abs(x)))) * (0.284496736 - ((1.0 / (1.0 + (0.3275911 * Math.abs(x)))) * ((1.421413741 + (1.061405429 * (1.0 / pow((1.0 + (0.3275911 * Math.abs(x))), 2.0)))) + ((1.0 / (1.0 + (0.3275911 * Math.abs(x)))) * -1.453152027))))))));
}
